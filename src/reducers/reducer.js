...
const initialState = {
  lists: [
    ...
    },
    {
      id: 2,
      title: 'In Progress',
      cards: [{ id: 3, text: 'Get Chocolates' }],
    },
    {
      id: 3,
      title: 'Done',
...
const getListParentIndexFromCardId = (newStateDnd, droppableIdStart) => {
  return newStateDnd.lists
    .map((list) => String(list.id))
    .indexOf(droppableIdStart);
};
    ...
    case actionDispatch.ORDER_CARD:
      const newStateDnd = { ...state };
      {
        ...
        if (droppableIdStart === droppableIdEnd) {
          const cardsArray =
            newStateDnd.lists[
              getListParentIndexFromCardId(newStateDnd, droppableIdStart)
            ].cards;
          const [removed] = cardsArray.splice(droppableIndexStart, 1);
          cardsArray.splice(droppableIndexEnd, 0, removed);
        } else {
          const cardsArraySource =
            newStateDnd.lists[
              getListParentIndexFromCardId(newStateDnd, droppableIdStart)
            ].cards;
          const cardsArrayDestination =
            newStateDnd.lists[
              getListParentIndexFromCardId(newStateDnd, droppableIdEnd)
            ].cards;
